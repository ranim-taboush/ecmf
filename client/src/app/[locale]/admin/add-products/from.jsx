          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 item-center justify-between">
              <Input required label={locale === 'ar' ? langAr.thickness : langEn.thickness} name='from' style={{maxWidth: "97%"}}
                placeholder={(locale === 'ar' ? langAr.placeholder : langEn.placeholder) + (locale === 'ar' ? langAr.from : langEn.from)} value={data.thickness?.from || ''} 
                onChange={(e)=>{ handleChange(e,'thickness', 'from')} } />
              <Input required name='to' label={'  '} style={{maxWidth: "97%"}}
                placeholder={(locale === 'ar' ? langAr.placeholder : langEn.placeholder) + (locale === 'ar' ? langAr.to : langEn.to)} value={data.thickness?.to || ''} 
                onChange={(e)=>{ handleChange(e,'thickness', 'to')} } />
            </div>
            <div className="grid grid-cols-2 item-center justify-between">
              <Input required name='arUnit' className="mr-2" style={{maxWidth: "97%"}}  
                placeholder={(locale === 'ar' ? langAr.placeholder : langEn.placeholder) + (locale === 'ar' ? langAr.unitAr : langEn.unitAr)} value={data.thickness?.arUnit || ''} 
                onChange={(e)=>{ handleChange(e,'thickness', 'arUnit')} } />
              <Input required name='enUnit'  style={{maxWidth: "97%"}}
                placeholder={(locale === 'ar' ? langAr.placeholder : langEn.placeholder) + (locale === 'ar' ? langAr.unitEn : langEn.unitEn)} value={data.thickness?.enUnit || ''} 
